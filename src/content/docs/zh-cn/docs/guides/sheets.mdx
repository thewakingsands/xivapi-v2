...
[Ensuring Stability]: /zh-cn/docs/guides/pinning/

## 语言

包含面向用户字符串的表格通常本地化为游戏客户端支持的所有语言。有关本地化和游戏版本的更多详细信息，请参阅[重要概念][concepts-localisations]。

虽然 XIVAPI 在响应中默认为英语文本，但可以使用 `language` 参数请求其他语言。
...
表格通常包含比单个用例所需更多的信息。当存在链接其他数据行的字段关系时，这会迅速增加。例如，"Angel Brush" `Item` 的[完整响应][pct brush full]在压缩前超过 80kB，代表大约 6500 行格式化的 JSON。
